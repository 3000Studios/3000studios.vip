...
const stories = [
  ...
  { id: officialReleaseVideos[3]?.videoId || featured.videoId, label: 'Merch', to: '/sponsors' },
...
export function DiscoverHome() {
  ...
  const buyTrack = (slug: string, title: string) => {
    grantTrack(slug);
    window.location.href = checkoutMailto('track', title);
  };
  ...
  return (
    <PublicLayout variant="spiral">
      <main className="discoverPage">
        ...
        <VideoWallpaperCard
          className="discoverHero"
          videoId={featured.videoId}
          kicker="Featured official video"
          title="Watch the videos. Own the drop."
        ...
        >
          ...
          <div className="discoverHeroActions">
            ...
            <span className="pill gold">Buy {formatMoney(TRACK_PRICE_CENTS)}</span>
          </div>
        </VideoWallpaperCard>
        ...
        <section className="discoverUnlock">
          ...
          <div className="unlockRow">
            <button type="button" className="studioButton" onClick={() => { grantPlan('monthly'); window.location.href = checkoutMailto('monthly'); }}>
              {formatMoney(MONTHLY_PRICE_CENTS)} / month
            </button>
            <button type="button" className="studioButton secondary" onClick={() => { grantPlan('yearly'); window.location.href = checkoutMailto('yearly'); }}>
            ...
            {rolloutSongs.slice(0, 18).map((song) => {
              ...
              return (
                <article className="sampleRow" key={song.slug}>
                  ...
                  {!unlocked ? (
                    <button type="button" className="buyBtn" onClick={() => buyTrack(song.slug, song.title)}>
