...
function Waveform() {
  return (
    <div className="deckWaveform" aria-label="Live audio visualizer">
      {Array.from({ length: 36 }, (_, index) => (
        <i key={index} style={{ '--bar': ((index * 17) % 11) / 10 } as CSSProperties} />
      ))}
    </div>
  );
}
...
export function MusicDeck() {
  ...
  const openOfficialVideo = () => {
    if (!video) return;
    window.dispatchEvent(new CustomEvent('3000-analytics', { detail: { event: 'official_video_click', videoId: video.videoId, title: video.title } }));
    const gtag = (window as Window & { gtag?: (...args: unknown[]) => void }).gtag;
    gtag?.('event', 'official_video_click', { video_id: video.videoId, song_title: video.title });
    window.open(youtubeWatchUrl(video.videoId), '_blank', 'noopener,noreferrer');
  };
    ...
    <PublicLayout variant="electric">
      <main className="musicDeckPage">
        <section className="deckArtworkStage">
          ...
          {video ? (
            <button className="officialVideoPrompt" type="button" onClick={openOfficialVideo}>
              <span>Official video</span>
              <strong><Play weight="fill" /> Watch on YouTube <ArrowSquareOut /></strong>
            </button>
          ) : (
            <div className="visualizerBadge"><span>Audio visualizer</span><Waveform /></div>
          )}
        </section>
        ...
        <section className={`turntableDeck${scratching ? ' is-scratching' : ''}`} aria-label="Interactive turntable">
          ...
          <button
            ...
            aria-label="Scratch record to seek; use left and right arrow keys to move five seconds"
          >
            <span className="vinylLabel"><img src="/media/official-3000-studios-profile.png" alt="" /></span>
            <span className="scratchHint">Drag to scratch</span>
          </button>
          <div className="deckLights" aria-hidden="true"><i /><i /><i /><i /><i /></div>
        </section>

        <section className="deckControls" aria-label="Music controls">
          <div className="deckTitle"><div><h1>{music.activeSong.title}</h1><p>3000 Studios</p></div>{video ? <button type="button" onClick={openOfficialVideo} aria-label="Open official video"><ArrowSquareOut /></button> : null}</div>
          <label className="deckSeek"><span className="srOnly">Song position</span><input type="range" min="0" max={music.duration || 1} value={Math.min(music.currentTime, music.duration || 1)} onChange={(event) => music.seekTo(Number(event.target.value))} /><small>{formatTime(music.currentTime)}</small><small>-{formatTime(Math.max(0, music.duration - music.currentTime))}</small></label>
          <div className="transportControls">
            <button type="button" onClick={music.prev} aria-label="Previous song"><SkipBack weight="fill" /></button>
            <button className="mainTransport" type="button" onClick={music.toggle} aria-label={music.isPlaying ? 'Pause' : 'Play'}>{music.isPlaying ? <Pause weight="fill" /> : <Play weight="fill" />}</button>
            <button type="button" onClick={music.next} aria-label="Next song"><SkipForward weight="fill" /></button>
          </div>
          <label className="volumeControl"><button type="button" onClick={() => music.setMuted(!music.muted)} aria-label={music.muted ? 'Unmute' : 'Mute'}>{music.muted ? <SpeakerSlash /> : <SpeakerHigh />}</button><span className="srOnly">Volume</span><input type="range" min="0" max="1" step="0.01" value={music.muted ? 0 : music.volume} onChange={(event) => music.setVolume(Number(event.target.value))} /></label>
        </section>

        <section className="deckQueue" aria-labelledby="up-next-title">
          <div className="queueHeading"><h2 id="up-next-title">Official release deck</h2><span>Swipe or use controls</span></div>
          <div className="releaseRail">
            {distrokidSongs.map((song) => {
              ...
              <button className={song.slug === music.activeSong.slug ? 'releaseTile is-active' : 'releaseTile'} type="button" key={song.slug} onClick={() => music.playIndex(index)}>
                <img src={song.cover} alt="" loading="lazy" /><span><strong>{song.title}</strong><small>3000 Studios</small></span><Play weight="fill" />
              </button>
            )})}
          </div>
        </section>
