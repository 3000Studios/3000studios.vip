...
describe('live access authorization', () => {
  ...
  it('creates a refresh-safe signed session and rejects tampering or expiry', async () => {
    ...
    await expect(hasLiveSession(request, env, now + 60_000)).resolves.toBe(true);
    const tampered = new Request('https://3000studios.vip/live', {
      headers: { cookie: `__Host-live_access=${session}x` },
    });
    await expect(hasLiveSession(tampered, env, now + 60_000)).resolves.toBe(false);
    await expect(hasLiveSession(request, env, now + 9 * 60 * 60 * 1000)).resolves.toBe(false);
  });
